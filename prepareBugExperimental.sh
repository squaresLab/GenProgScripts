...
MODIFIEDCLASSESLIST=posTestsRemoved.tests
...
#line for debugging
#echo "$D4J_HOME/$BUGSFOLDER/$LOWERCASEPACKAGE$2Buggy/${12}"
#exit 0
...
classTestFolder = $D4J_HOME/$BUGSFOLDER/$LOWERCASEPACKAGE$2Buggy/${TESTFOLDER}
libs = $CONFIGLIBS
sourceDir = $WD
...
#else
  #not used due to Zhen's preprocessing step, which separates failing test cases into its own class(es)
