...
if [ -d "$GP4J_HOME" ]; then
  ...
  #sudo update-java-alternatives -s $DIROFJAVA7
	...
	tar -cvf $D4J_HOME/$BUGSFOLDER/"$LOWERCASEPACKAGE""$BUGNUMBER"Buggy/variants"$PROJECT""$BUGNUMBER"Seed$seed.tar $D4J_HOME/$BUGSFOLDER/"$LOWERCASEPACKAGE""$BUGNUMBER"Buggy/tmp/ > /dev/null
	...
	#rm $D4J_HOME/$BUGSFOLDER/"$LOWERCASEPACKAGE""$BUGNUMBER"Buggy/*.ser
      done
    fi
fi
